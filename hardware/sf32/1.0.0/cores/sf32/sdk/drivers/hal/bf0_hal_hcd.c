        ...
        uint8_t do_ping)
{
    ...
    if (direction == 0)     // TX
    {
        ...
        else
        {
            ...
            //HAL_DBG_printf("TX ch_num=%d,Address:%d\n",ch_num,hhcd->hc[ch_num].dev_addr);
            csr = epn->txcsr;
            __IO uint8_t *fifox = (__IO uint8_t *) & (hhcd->Instance->fifox[ep_num]);
            ...
            epn->txtype = (hhcd->hc[ch_num].ep_type << 4) + hhcd->hc[ch_num].ep_type;
            epn->txinterval = (HAL_HCD_GetCurrentSpeed(hhcd) == HCD_SPEED_HIGH ? 16 : 2);
            epn->txcsr = csr;
            //HAL_DBG_printf("TX ep %d: csr=0x%x, len=%d", ep_num, csr, length);
            //HAL_DBG_printf("mps=%d, type=%d, txintval=%d\n", epn->txmaxp, epn->txtype, epn->txinterval);
        }
    }
    else
    {
        ...
        else
        {
            ...
            //HAL_DBG_printf("RX ch_num=%d,Address:%d\n",ch_num,hhcd->hc[ch_num].dev_addr);
            csr = epn->rxcsr;
            ...
            //HAL_DBG_printf("RX ep %d: csr=0x%x, len=%d, ", ep_num, csr, length);
            //HAL_DBG_printf("mps=%d, type=%d, txintval=%d\n", epn->rxmaxp, epn->rxtype, epn->rxinterval);
            epn->rxcsr = csr;
...
static void HCD_HC_IN_IRQHandler(HCD_HandleTypeDef *hhcd, uint8_t ep_num)
{
    ...
    else
    {
        ...
        //HAL_DBG_printf("rx complete2:count=%d, rxcsr=%x\n", epn->rxcount, epn->rxcsr);
    }
...
static void HCD_HC_OUT_IRQHandler(HCD_HandleTypeDef *hhcd, uint8_t ep_num)
{
    ...
    if (chnum < 0)
        return;
    //HAL_DBG_printf("tx complete: ep_num=%d,chnum=%d,csr=0x%x\n", ep_num, chnum, txcsr);
